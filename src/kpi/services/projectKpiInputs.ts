...
export async function getProjectBaselineForKpi(
    ...
    if (!Number.isInteger(projectId) || projectId <= 0) throw new InvalidProjectDataError('projectId must be a positive integer.');
    ...
    const bac = project.plannedBudget instanceof Prisma.Decimal
    ...
    if (endDate.getTime() <= startDate.getTime()) {
        throw new InvalidProjectDataError("Invalid baseline interval: endDate must be after startDate.");
    }
