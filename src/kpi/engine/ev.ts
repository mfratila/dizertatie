...
export function computeProgressEqualWeighted(items: WorkItemProgress[]): ProgressResult {
    ...
    for (const item of items) {
        ...
        if (!Number.isFinite(progress)) throw new InvalidProgressError('progressPercent must be a finite number.');
        if (progress < 0 || progress > 100) throw new InvalidProgressError('progressPercent must be in [0, 100] range.');
...
export function calculateEV(bac: number, progressRatio: number): number {
    ...
    if (!Number.isFinite(progressRatio)) throw new InvalidProgressError('progressRation must be a finite number.');
    if (progressRatio < 0 || progressRatio > 1) throw new InvalidProgressError('progressRatio must be in [0, 1] range.');
